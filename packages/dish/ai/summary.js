 ...
 * - 生成:streamText(messages) 混元 hy3,system 约束只许使用给定数据,120 字内(流式回显)
...
Page({
  ...
  data: {
    ...
    loading: false, // 生成中
    summaryText: '', // 生成结果
    summaryStream: '', // 流式回显(生成中实时累计,完成后清空)
  ...
  async generate() {
    ...
    this.setData({ loading: true, error: '', summaryText: '', summaryStream: '' });
    try {
      ...
      const text = await streamText(
        ...
        ],
        {
          ...
          onChunk: (chunk) => {
            this.setData({ summaryStream: this.data.summaryStream + chunk });
          },
        },
      );
      this.setData({ loading: false, summaryText: text, summaryStream: '' });
    } catch (err) {
      console.error('AI 小结生成失败', err);
      this.setData({ loading: false, error: err.message || '生成失败，请重试', summaryStream: '' });
