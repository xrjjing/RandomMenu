...
| 按料找菜 | 首页勾选「冰箱里有什么」，支持交集匹配（按匹配度排序）与完全匹配两种模式 |
| 随机转盘 | 从候选菜品中 Canvas 绘制转盘随机抽取，确认后直接落账 |
| 做菜记录 | 每日落账可撤销；按日 / 月 / 年统计，柱状图 / 饼图展示，附原料使用榜 |

## 技术栈

原生微信小程序 · TDesign miniprogram 组件库 · 微信云开发文档数据库 + 云存储 · Less

- 零云函数：数据库读写全部由小程序端 SDK 直连（集合权限全开，见「快速开始」）
- 图表与转盘均为 Canvas 2D 自绘组件，无外部图表依赖
...
├── packages/            # 分包（ingredient 原料库 / dish 菜品详情与编辑）
